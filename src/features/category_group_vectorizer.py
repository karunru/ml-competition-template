...
class CategoryGroupVectorization(Feature):
    def create_features(
        self,
        ...
        with timer("concat train and test"):
            ...
            total = xfeat.utils.compress_df(total, verbose=True)
            gc.collect()
            total.columns = [
        ...
        with timer("category vectorize"):
            ...
            total = total[cat_features + ["customer_ID"]]
            gc.collect()
        ...
        for col_group in [
            ...
            cols = [
                ...
                if col.replace("denoise_", "").startswith(col_group)
            ]
            ...
            countsvd_vec_df = (
                CategoryUser2Vec(
                    ...
                    name=f"{col_group}_CountSVD",
                )
                .transform(total[cols + ["customer_ID"]])
